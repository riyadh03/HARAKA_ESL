...
import { Camera as CameraIcon, CheckCircle2, AlertCircle, Play, RefreshCw, Send, ArrowLeft, Eye, EyeOff } from "lucide-react";
...
>>>>>>> be0c2b1dc8111f1c63151993e19e988d8c5df04b
  ...
  const isPrivacyModeRef = useRef(true);
...
<<<<<<< HEAD
          ...
          if (isPrivacyModeRef.current) {
            // MediaPipe Facial Landmarks are typically indices 0 through 10
            const faceLandmarks = results.poseLandmarks.slice(0, 11);
            
            if (faceLandmarks.length > 0 && faceLandmarks[0].visibility > 0.5) {
              // Find the bounding box of the face
              const xs = faceLandmarks.map((lm: any) => lm.x * canvasRef.current!.width);
              const ys = faceLandmarks.map((lm: any) => lm.y * canvasRef.current!.height);
              ...
              // Calculate radius based on face width/height, add padding
              const radius = Math.max((maxX - minX), (maxY - minY)) * 1.2;
              
              canvasCtx.save();
              canvasCtx.beginPath();
              canvasCtx.arc(centerX, centerY, Math.max(radius, 40), 0, 2 * Math.PI);
              canvasCtx.fillStyle = 'rgba(15, 23, 42, 0.95)'; // Dark slate blur color
              canvasCtx.shadowColor = 'rgba(15, 23, 42, 1)';
...
<<<<<<< HEAD
        <div className="flex items-center gap-3">
          <button
            ...
            className={`px-4 py-2 rounded-full flex items-center gap-2 text-sm font-bold shadow-sm transition-colors duration-300 ${isPrivacyMode ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            {isPrivacyMode ? <EyeOff size={18} /> : <Eye size={18} />}
            <span>{isPrivacyMode ? 'Anonymat Activé' : 'Mode Privé'}</span>
          </button>
